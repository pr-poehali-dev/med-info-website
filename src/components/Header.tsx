...
const navItems = [
  ...
  { id: 'info', label: 'Информация', submenu: [{ id: 'diseases', label: 'Заболевания и синдромы' }] },
                ...
                >
                  <button
                    className={`nav-link px-4 py-2 rounded-lg text-sm font-medium font-golos transition-all duration-200 flex items-center gap-1 hover:bg-sage/10 ${
                      currentPage === 'info' || currentPage === 'diseases' ? 'text-sage active' : 'text-dark-text'
                    }`}
                  >
                    {item.label}
                  ...
                  {submenuOpen && (
                    <div className="absolute top-full left-0 mt-1 w-52 glass rounded-xl shadow-xl border border-white/40 py-2 animate-scale-in">
                      {item.submenu.map((sub) => (
                        <button
                          key={sub.id}
                          onClick={() => { onNavigate(sub.id); setSubmenuOpen(false); }}
                          className="w-full text-left px-4 py-2 text-sm text-dark-text hover:bg-sage/10 hover:text-sage transition-colors font-golos"
                        >
                          {sub.label}
                        </button>
                      ))}
                    ...
                    <div className="ml-4 mt-1 flex flex-col gap-1">
                      {item.submenu.map((sub) => (
                        <button
                          ...
                          className="text-left px-4 py-2 rounded-lg text-sm text-light-text hover:bg-sage/10 hover:text-sage font-golos"
                        >
                          {sub.label}
                        </button>
                      ))}
